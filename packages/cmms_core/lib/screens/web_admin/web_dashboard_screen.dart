...
class WebDashboardScreen extends StatelessWidget {
  ...
  Widget _buildSfPieChart(List<WorkOrder> workOrders) {
    ...
    final colors = {
      for (final s in WorkOrderStatus.values) s: AppTheme.workOrderStatusColor(s),
    };
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color:
                          AppTheme.workOrderStatusColor(wo.status).withValues(alpha: 0.1),
                      borderRadius: BorderRadius.circular(8),
                    ),
                    child: Icon(
                      Icons.work_outline,
                      color: AppTheme.workOrderStatusColor(wo.status),
                      size: 20,
                    ),
                  ),
  ...
  Widget _buildStatusBadge(WorkOrderStatus status) => Container(
        ...
        decoration: BoxDecoration(
          color: AppTheme.workOrderStatusColor(status).withValues(alpha: 0.1),
          borderRadius: BorderRadius.circular(12),
        ),
        child: Text(
