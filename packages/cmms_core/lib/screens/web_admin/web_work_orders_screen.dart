...
class _WebWorkOrdersScreenState extends State<WebWorkOrdersScreen> {
  ...
  Widget _buildStatusBadge(WorkOrderStatus status) {
    final colors = {
      ...
      WorkOrderStatus.reopened: const Color(0xFF002911), // Be Electric green for reopened
    };

    final statusColor = colors[status] ?? const Color(0xFF6B7280); // Default to grey if not found
    ...
    return Container(
      ...
      child: Text(
        status.name == 'reopened' ? 'REOPENED' : status.name.toUpperCase(),
        style: TextStyle(
          fontSize: 11,
          fontWeight: FontWeight.w600,
          color: statusColor,
